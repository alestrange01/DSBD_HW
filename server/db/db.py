...
class DB:
    ...
    def initialize_database(cls):
        ...
        if not users:
            ...
            admin_user = user_repository_writer.UserCreationDTO("admin@gmail.com", bcrypt.hashpw("admin".encode('utf-8'), bcrypt.gensalt()).decode('utf-8'), "AAPL", "admin", 250.00, 220.00)
            user_user = user_repository_writer.UserCreationDTO("user1@gmail.com", bcrypt.hashpw("user1".encode('utf-8'), bcrypt.gensalt()).decode('utf-8'), "TSLA", "user", 377.93, 327.93)
            ...
            ticker_aapl = ticker_management_repository_writer.TickerManagementDTO("AAPL", 1)
            ticker_tsla = ticker_management_repository_writer.TickerManagementDTO("TSLA", 1)
