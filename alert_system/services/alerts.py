        ...
        for user in users:
            ...
            if latest_share:
                ...
                if high_value_exceeded or low_value_exceeded:
                    ...
                    message = {
                        "to": user.email,
                        "subject": "Valore al limite:" + str(user.share_name),
...
def handle_message(msg):
    try:
        ...
        logging.info(f"Consumed: {data}")
        if process(data):
            consumer.commit(asynchronous=False) #TODO Il commit non va fatto a priori?
